...
class Torso(object):
    def __init__(self):
        ...
        self.eef_pub_l = rospy.Publisher('/nips2017/torso/left_arm/end_effector_pose', PoseStamped, queue_size=1)
        self.eef_pub_r = rospy.Publisher('/nips2017/torso/right_arm/end_effector_pose', PoseStamped, queue_size=1)
        self.js_pub_l = rospy.Publisher('/nips2017/torso/left_arm/joints', JointState, queue_size=1)
    ...
    def go_to_rest(self, slow):
        with self.robot_lock:
            ...
            self.torso.goto_position({'l_elbow_y': -35, 'l_shoulder_x': 30}, duration)
            rospy.sleep(duration)
    ...
    def run(self, dummy=False):
        ...
        try:
            ...
            self.srv_reset = rospy.Service('/nips2017/torso/reset', Reset, self._cb_reset)
            self.srv_execute = rospy.Service('/nips2017/torso/execute', ExecuteTorsoTrajectory, self._cb_execute)
            self.srv_set_compliant = rospy.Service('/nips2017/torso/set_compliant', SetTorsoCompliant, self._cb_set_compliant)
